...
- **Design discussion is a phase, not a separate tracker record.** `create-design-discussion/SKILL.md` frames it as an early working document that consolidates product + technical thinking into a short form of PRD+TDD, written before either exists (all questions start in the OPEN state and the user resolves them). The prototype therefore does **not** materialize a `design:discussion` record — only its most distinctive output, the **Proposed End-State Architecture Before/After diagrams**, is carried inside each TDD's **System Design** (where the delta story already lives). Decisions made during that phase are baked into the PRD/TDD text (e.g. the Large TDD records the soft-float and CubeMX-HAL decisions inside the "one platform swap" takeaway).
...
- **Technical side** follows `create-tdd/SKILL.md` + `create-tdd/references/tdd_template.md`:
  ...
  - **Patterns to Follow** — existing codebase patterns with file locations and snippets, in the precise form the design-discussion template uses: subject line, `Pattern:` / `File:` fields, and an existing→proposed **before/after code pair** (or a single snippet for a genuinely new pattern). "Do X" became "here is the existing code and here is the proposed shape".
...
4. **Template fidelity** — does each TDD read like a HumanLayer TDD (takeaway headers, diagrams beside prose, seams and deltas made explicit)? Does the product side read like a HumanLayer PRD? Do the Before/After diagrams inside System Design make the migration visible at a glance (Large and Medium), and does Small's one-diagram pair carry the delta without ceremony? Do the Pattern before/after pairs make the *new* code shape concrete rather than aspirational?
