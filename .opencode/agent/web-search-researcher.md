...
- use the `bash` tool to `curl -sL` any pertinent documentation links from project instructions which end in `llms.txt`
...
- if you know the URL / site for an app (e.g. `https://vite.dev`), you can _always_ try curl-ing `https://<site>/llms.txt` to see if a `llms.txt` file is available. It may or may not be, but you should always check since it is a valuable source optimized for language models.
